...
def load_model_and_tokenizer(model_args: ModelArguments, lora_config: LoraConfig):
    ...
    # NOTE: This is LoRA-based supervised fine-tuning (SFT). No reward model or KL regularization here.
...
def train_stage(
    ...
    # Save tokenizer with the stage so downstream eval can load it directly
    tokenizer.save_pretrained(training_args.output_dir)
    
    # Optionally save a merged full model (when requested)
    if hasattr(trainer.model, "merge_and_unload") and trainer.args is not None and getattr(trainer.args, "output_dir", None):
        if getattr(training_args, "save_total_limit", None) is not None:
            pass  # keep HF behavior
        if getattr(cli_args, "save_merged_full_model", False):
            merge_dir = os.path.join(training_args.output_dir, "merged-full")
            os.makedirs(merge_dir, exist_ok=True)
            merged = trainer.model.merge_and_unload()
            merged.save_pretrained(merge_dir)
            tokenizer.save_pretrained(merge_dir)
            logger.info(f"Merged full model saved to: {merge_dir}")
...
def main():
    ...
    parser.add_argument("--save_merged_full_model", action="store_true", help="Also save a merged, full-weights model per stage")
    ...
    # Define the three training stages in curriculum order
    stages = [
        {
            "name": "pubmedqa",
    ...
    for stage_idx in stages_to_run:
        ...
        if model is None:
            if stage_idx > 0 and args.stage == "all":
                ...
                # Save the base tokenizer into stage 1 output when created
                os.makedirs(stages[stage_idx]["output_dir"], exist_ok=True)
                tokenizer.save_pretrained(stages[stage_idx]["output_dir"])
