...
# Fine-tuned model and tokenizer.      
    def _load_model(self) -> Tuple[torch.nn.Module, Any]:
        ...
        # Try to load tokenizer from the fine-tuned folder first (it was saved during training)
        try:
...
# Extracting the answer choice from both predicted and reference.    
    ...
    def evaluate_sample(self, sample: Dict, sample_idx: int) -> Dict:
        ...
        is_mcq = bool(choices) or bool(re.search(r'\b[abcd]\)', instruction.lower()))
    ...
    def evaluate_dataset(
        ...
        accuracy = float(np.mean([r["accuracy"] for r in detailed_results]))
        exact_match = float(np.mean([r["exact_match"] for r in detailed_results]))
        bleu_score = float(np.mean([r["bleu_score"] for r in detailed_results]))
        rouge_1 = float(np.mean([r["rouge_1"] for r in detailed_results]))
        rouge_2 = float(np.mean([r["rouge_2"] for r in detailed_results]))
        rouge_l = float(np.mean([r["rouge_l"] for r in detailed_results]))
        avg_response_length = float(np.mean([r["response_length"] for r in detailed_results]))
        avg_inference_time = float(np.mean([r["inference_time"] for r in detailed_results]))
...
def main():
    ...
    if args.batch_eval:
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"- ROUGE-1: {results.rouge_1:.4f}\n")
            f.write(f"- ROUGE-2: {results.rouge_2:.4f}\n")
            f.write(f"- ROUGE-L: {results.rouge_l:.4f}\n\n")
            
            f.write(f"Efficiency:\n")
            f.write(f"- Avg Response Length: {results.avg_response_length:.1f} words\n")
